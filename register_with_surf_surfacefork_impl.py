...
from fireants.utils.orientation import OrientationContext, sampling_grid_to_original

ALIGN_CORNERS = False


def contiguous_image_data(image, dtype=None):
    return np.ascontiguousarray(np.asarray(image.dataobj, dtype=dtype))
...
def crop_image_to_mask(img, mask_data, margin=9):
    ...
    data = contiguous_image_data(img)
...
def save_in_original_orientation(data, orientation, filename):
    save_vol_nii(
        orientation.restore_array(data),
        orientation.original.affine,
        filename,
    )
...
def main(args):
    ...
    src_vol_input = nib.load(args.src_vol)
    trg_vol_input = nib.load(args.trg_vol)
    reorient_enabled = not getattr(args, "no_reorient", False)
    src_orientation = OrientationContext.create(
        src_vol_input,
        enabled=reorient_enabled,
    )
    trg_orientation = OrientationContext.create(
        trg_vol_input,
        enabled=reorient_enabled,
    )
    src_vol_orig = src_orientation.working
    trg_vol_orig = trg_orientation.working
    print(
        "  source orientation: "
        f"{''.join(src_orientation.original_axcodes)} -> "
        f"{''.join(src_orientation.working_axcodes)}"
    )
    print(
        "  target orientation: "
        f"{''.join(trg_orientation.original_axcodes)} -> "
        f"{''.join(trg_orientation.working_axcodes)}"
    )

    src_mask_img = (
        src_orientation.reorient_image(
            nib.load(args.src_mask),
            name="source mask",
        )
        if args.src_mask
        else None
    ...
    trg_mask_img = (
        trg_orientation.reorient_image(
            nib.load(args.trg_mask),
            name="target mask",
        )
        if args.trg_mask
        else None
    ...
    if args.src_mask:
        src_mask_data = contiguous_image_data(src_mask_img, dtype=np.float32)
    ...
    if args.trg_mask:
        trg_mask_data = contiguous_image_data(trg_mask_img, dtype=np.float32)
    ...
    ###################
    if args.src_lbl:
        src_lbl_img = src_lbl_orig
        trg_lbl_img = trg_lbl_orig
        ...
        if trg_crop is not None and trg_lbl_img is not None:
        ...
        moving_label = torch.from_numpy(
            contiguous_image_data(src_lbl_img)
        ).long()[None][None].to(device)
        fixed_label = (
            torch.from_numpy(
                contiguous_image_data(trg_lbl_img)
            ).long()[None][None].to(device)
            if trg_lbl_img is not None
            else None
        )
    else:
        moving_label = None
    ...
    if nonlinear_only:
        if src_vol.shape != trg_vol.shape or not np.allclose(
            src_vol.affine, trg_vol.affine, atol=1e-4
        ):
            raise ValueError(
                "--nonlinear_only requires source and target volumes on the same grid"
            )
        print("Nonlinear-only mode: using identity affine initialization.")
        init_affine = torch.eye(
            4,
            device=device,
    ...
    ###################
    # Save/check affine-only alignment
    ###################
    out_affine_vol = getattr(args, "out_affine_vol", None)
    out_affine_lbl = getattr(args, "out_affine_lbl", None)
    out_affine_surf = getattr(args, "out_affine_surf", None)
    affine_only = getattr(args, "affine_only", False)

    if out_affine_vol or out_affine_lbl or out_affine_surf or affine_only:
        has_crop = (trg_crop is not None) or (src_crop is not None)

        if has_crop:
            zero_disp = torch.zeros(
                init_affine.shape[0],
                *trg_vol.shape[:3],
                3,
                device=init_affine.device,
            ...
            affine_warp_grid = build_full_forward_warp_grid(
                fwd_disp_crop=zero_disp,
            ...
            affine_warp_grid = F.affine_grid(
                init_affine[:, :-1, :],
                torch.Size((init_affine.shape[0], 1, *trg_vol_orig.shape[:3])),
                align_corners=ALIGN_CORNERS,
            )

        if out_affine_vol:
            moving_full_data = contiguous_image_data(src_vol_orig, dtype=np.float32)
            moving_full_tensor = torch.from_numpy(moving_full_data)[None, None].to(device)
            moving_affine_warped = F.grid_sample(
                moving_full_tensor,
                affine_warp_grid,
                mode="bilinear",
                padding_mode="border",
            ...
            save_in_original_orientation(
                moving_affine_warped[0, 0].detach().cpu().numpy(),
                trg_orientation,
                out_affine_vol,
            )
            del moving_full_tensor, moving_affine_warped

        if out_affine_lbl:
            if not args.src_lbl:
                raise ValueError("--out_affine_lbl requires --src_lbl")
            moving_label_full = torch.from_numpy(
                contiguous_image_data(src_lbl_orig, dtype=np.float32)
            )[None, None].to(device)
            moving_label_affine = F.grid_sample(
                moving_label_full,
                affine_warp_grid,
                mode="nearest",
                padding_mode="zeros",
            ...
            save_in_original_orientation(
                moving_label_affine[0, 0].detach().cpu().numpy(),
                trg_orientation,
                out_affine_lbl,
            )
            del moving_label_full, moving_label_affine
        if out_affine_surf:
            if not args.src_surf:
                raise ValueError("--out_affine_surf requires --src_surf")

            # init_affine is fixed-normalized -> moving-normalized sampling.
            # Surface output uses its inverse, moving voxels -> fixed voxels.
            moving_xyz = moving_surf.flip(dims=[-1])
            moving_norm = vox_to_norm(
                moving_xyz,
                src_vol.shape[:3],
                align_corners=ALIGN_CORNERS,
            )
            ones = torch.ones(
                *moving_norm.shape[:-1],
                1,
                device=moving_norm.device,
                dtype=moving_norm.dtype,
            )
            fixed_norm = torch.bmm(
                inv_init_affine,
                torch.cat((moving_norm, ones), dim=-1).transpose(1, 2),
            ).transpose(1, 2)[..., :3]
            fixed_ijk = norm_to_vox(
                fixed_norm,
                trg_vol.shape[:3],
                align_corners=ALIGN_CORNERS,
            ).flip(dims=[-1])
            if trg_crop is not None:
                fixed_ijk = fixed_ijk + torch.tensor(
                    [trg_crop["i_min"], trg_crop["j_min"], trg_crop["k_min"]],
                    device=fixed_ijk.device,
                    dtype=fixed_ijk.dtype,
                ).view(1, 1, 3)
            affine_surface_world = vox2phy(
                fixed_ijk,
                torch.as_tensor(
                    trg_vol_orig.affine,
                    device=fixed_ijk.device,
            ...
            save_surf_gii(
                affine_surface_world,
                src_surf.faces_packed(),
                out_affine_surf,
            )

        if affine_only:
            print("Affine-only mode requested; skipping nonlinear registration.")
            return

        del affine_warp_grid
    ...
    reg = GreedyRegistration(
        ...
        surf_weight=args.surface_weight,
    ...
    if has_crop:
        ...
        _src_data = contiguous_image_data(src_vol_orig, dtype=np.float32)
        _trg_data = contiguous_image_data(trg_vol_orig, dtype=np.float32)
        ...
        if args.src_lbl:
            moving_label = torch.from_numpy(
                contiguous_image_data(src_lbl_orig)
            ).long()[None][None].to(device)
    ...
    ###################
    if args.out_warp:
        export_fwd_grid = sampling_grid_to_original(
            fwd_warp_grid,
            trg_orientation,
            src_orientation,
        )
        if args.warp_format == "ants":
            disp = grid_to_ants_displacement(
                export_fwd_grid,
                trg_orientation.original,
                src_orientation.original,
            )
            save_ants_displacement(
                disp,
                trg_orientation.original,
    ...
    if args.out_inv_warp:
        export_rev_grid = sampling_grid_to_original(
            rev_warp_grid,
            src_orientation,
            trg_orientation,
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Output affine-only surface file name",
    )
    parser.add_argument(
        "--out_warp",
        type=str,
