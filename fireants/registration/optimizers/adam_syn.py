...
class WarpAdam:
    ...
    '''
    def __init__(self, warp, inv_warp, lr, 
        ...
        self.inv_half_resolution = 1.0/(max(inv_warp.shape[1:-1]) - 1)
        self.warp = warp
        ...
        self.exp_avg_inv = torch.zeros_like(inv_warp, device=self.device if not self.offload else 'cpu')
        self.exp_avg_sq_inv = torch.zeros_like(inv_warp, device=self.device if not self.offload else 'cpu')
        ...
        self.initialize_grid(warp.shape[1:-1], inv_warp.shape[1:-1])
    ...
    def set_data_and_size(self, warp, size, inv_warp, inv_size, grid_copy=None):
        ...
        if any([s != size[i] for i, s in enumerate(self.exp_avg.shape[1:-1])]):
            if self.offload:
                ...
                self.exp_avg_inv = self.exp_avg_inv.to(self.device)
                self.exp_avg_sq_inv = self.exp_avg_sq_inv.to(self.device)
            ...
            if self.reset:
                ...
                inv_warp_size = [self.batch_size, *inv_size, self.n_dims]
                self.exp_avg_inv = torch.zeros(inv_warp_size, device=self.device if not self.offload else 'cpu')
                self.exp_avg_sq_inv = torch.zeros(inv_warp_size, device=self.device if not self.offload else 'cpu')
            else:
                ...
                self.exp_avg_inv = F.interpolate(self.exp_avg_inv.detach().permute(*self.permute_vtoimg), size=inv_size, mode=mode, align_corners=True, 
                                    ).permute(*self.permute_imgtov)
                self.exp_avg_sq_inv = F.interpolate(self.exp_avg_sq_inv.detach().permute(*self.permute_vtoimg), size=inv_size, mode=mode, align_corners=True, 
                                    ).permute(*self.permute_imgtov)
            ...
            if self.offload:
                ...
                self.exp_avg_inv = self.exp_avg_inv.to('cpu')
                self.exp_avg_sq_inv = self.exp_avg_sq_inv.to('cpu')

        self.half_resolution = 1.0/(max(warp.shape[1:-1]) - 1)
        self.inv_half_resolution = 1.0/(max(inv_warp.shape[1:-1]) - 1)
        self.initialize_grid(size, inv_size, grid_copy=grid_copy)
        # print(self.warp.shape, warpinv)
    
    def initialize_grid(self, size, inv_size, grid_copy=None):
        ...
        if fireants_interpolator.use_ffo:
            ...
            if grid_copy is None:
                ...
                self.inv_grid = F.affine_grid(self.affine_init, [self.batch_size, 1, *inv_size], align_corners=True).detach()
            else:
                self.grid = grid_copy
    ...
    def augment_jacobian(self, u, warp_type='warp'):
        ...
        if fireants_interpolator.use_ffo:
            ...
            if warp_type == 'warp':
                jac = jacobian_fn(self.warp.data + self.grid, normalize=True)
            elif warp_type == 'inv_warp':
                jac = jacobian_fn(self.inv_warp.data + self.inv_grid, normalize=True)
            else:
    ...
    def diffeomorphic_grad(self, grad, warp, grid, affine_init, half_resolution):
        ...
        grad.div_(gradmax).mul_(half_resolution)
        
        # multiply by learning rate
        grad.mul_(-self.lr)

        ## grad = grad + warp * (x + grad)   # this is the compositional update
        diffeo_grad = fireants_interpolator.warp_composer(warp, affine=affine_init, v=grad, grid=grid, align_corners=False)
        return diffeo_grad
            
    def step(self):
        ''' check for momentum, and other things '''
        grad_warp = self.warp.grad.data
        grad_inv_warp = self.inv_warp.grad.data
        
        if self.multiply_jacobian:
            grad_warp = self.augment_jacobian(grad_warp, warp_type='warp')
            grad_inv_warp = self.augment_jacobian(grad_inv_warp, warp_type='inv_warp')
        
        # add weight decay term
        if self.weight_decay > 0:
            grad_warp.add_(self.warp.data, alpha=self.weight_decay)
            grad_inv_warp.add_(self.inv_warp.data, alpha=self.weight_decay)
        ...
        self.exp_avg.mul_(self.beta1).add_(grad_warp, alpha=1-self.beta1)
        self.exp_avg_sq.mul_(self.beta2).addcmul_(grad_warp, grad_warp.conj(), value=1-self.beta2)
        self.exp_avg_inv.mul_(self.beta1).add_(grad_inv_warp, alpha=1-self.beta1)
        self.exp_avg_sq_inv.mul_(self.beta2).addcmul_(grad_inv_warp, grad_inv_warp.conj(), value=1-self.beta2)
        ...
        self.adam_update_kernel(grad_warp, self.exp_avg, self.exp_avg_sq, beta_correction1, beta_correction2, self.eps)
        self.adam_update_kernel(grad_inv_warp, self.exp_avg_inv, self.exp_avg_sq_inv, beta_correction1, beta_correction2, self.eps)
        ...
        if self.freeform:
            grad_warp.mul_(-self.lr)
            grad_inv_warp.mul_(-self.lr)
            self.warp.data.add_(grad_warp)
            self.inv_warp.data.add_(grad_inv_warp)
        else:
            # This is the diffeomorphic update
            diffeo_grad_warp = self.diffeomorphic_grad(grad_warp, self.warp.data, self.grid, self.affine_init, self.half_resolution)
            diffeo_grad_inv_warp = self.diffeomorphic_grad(grad_inv_warp, self.inv_warp.data, self.inv_grid, self.affine_init, self.inv_half_resolution)

            mode = 'bilinear' if self.n_dims == 2 else 'trilinear'
            
            self.warp.data.copy_(grad_warp.add_(diffeo_grad_warp))
            self.inv_warp.data.copy_(grad_inv_warp.add_(diffeo_grad_inv_warp))
            
            # self.warp.data.copy_(
            #     grad_warp.add_(
            #         (diffeo_grad_warp - F.interpolate(
            #             diffeo_grad_inv_warp.detach().permute(*self.permute_vtoimg),
            #             size=grad_warp.shape[1:-1],
            #             mode=mode,
            #             align_corners=False
            #             ).permute(*self.permute_imgtov)
            #         )) / 2.0
            #     )
            # self.inv_warp.data.copy_(
            #     grad_inv_warp.add_(
            #         (diffeo_grad_inv_warp - F.interpolate(
            #             diffeo_grad_warp.detach().permute(*self.permute_vtoimg),
            #             size=grad_inv_warp.shape[1:-1],
            #             mode=mode,
            #             align_corners=False
            #             ).permute(*self.permute_imgtov)
            #         )) / 2.0
        ...
        if self.smoothing_gaussians is not None:
            ...
            self.inv_warp.data = self.smoothing_wrapper(self.inv_warp.data, self.smoothing_gaussians, self.padding_smoothing)
